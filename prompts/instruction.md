...
매일 한 번의 실행(**11:00 KST = 02:00 UTC**, arXiv `/new`가 올라온 뒤)이 생성과 Slack 발송을 함께 처리한다. 에이전트는 메시지 본문(mrkdwn)을 `out/slack_message.txt`로 작성하고, 같은 실행의 마지막 단계가 **push 성공 직후** 이를 **Slack 봇(`chat.postMessage`)**으로 `SLACK_CHANNEL_ID` 채널에 보낸다. **daily report 발행 시(`out/release_ok.txt` 존재)** 요약을, **실행 실패 시** 실패 알림을 보내며, **주말·no-op은 미발송**이다.
