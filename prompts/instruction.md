...
예:
- 월요일 daily 실행(11:00 KST)은 그날 daily(`post_date=월요일`)만 만든다.
- 지난주 회고 weekly는 **별도의 월요일 오전 weekly 실행**(10:00 KST, `mode=weekly`)이 만든다 — `posts/YYYY-MM-DD-weekly.html`(파일명 날짜 = 발행 월요일). 지난주(직전 월~금) 평일 daily가 모두 존재해야 하며, 없으면 그 주 daily 실행들의 Calendar Audit이 먼저 Backfill해 둔다.
...
### 4.2 Daily mode
조건:
- 평일 실행(월~금). 월요일도 daily를 생성한다. **Weekly는 daily 실행에서 만들지 않는다** — 별도의 월요일 오전 weekly 실행이 담당한다.
...
### 4.3 Weekly mode
조건:
- **매주 월요일 오전 10:00 KST의 독립 실행**(`mode=weekly`, `0 1 * * 1` cron). daily 실행(11:00 KST)과 분리돼 있으며, weekly 실행은 daily를 만들지 않는다.
- 지난주(직전 월~금) 평일 daily 산출물이 모두 존재해야 한다(없으면 그 주 daily 실행들의 Calendar Audit이 미리 Backfill).
...
- weekly는 **지난주(직전 월~금) 산출물만 회고**하며 당일(월) daily에 의존하지 않는다(당일 daily는 이후 11:00 실행이 생성). 따라서 당일 daily의 `insights`를 weekly 실행에서 만들지 않는다.
...
동작:
- arXiv 미발행이므로 아무 산출물도 만들지 않고 종료한다. (weekly는 월요일 오전 독립 weekly 실행이 발행)
...
**발행 시점 = 매주 월요일, 회고 대상 = 직전 주(지난주 월~금).** 따라서 본문에서 회고 대상 주는 "이번 주"가 아니라 **"지난주"**로 칭한다(예: "지난주는 ~로 기억된다", "지난주 판세"). 상단 메타에는 발행 월요일과 검토 기간(지난주 월~금)을 함께 표기한다. "다음 주"는 발행 시점 기준 이번 주를 가리킨다.

따라서 weekly는 아래 4가지 질문에 반드시 답해야 한다.

1. 지난주에 판세가 실제로 바뀐 축은 무엇인가?
...
1. 상단 홈 버튼
2. 🗓 Executive Summary — 지난주의 결론 3줄. 숫자 나열 금지, “그래서 지난주는 무엇으로 기억할지”를 쓴다.
3. 주간 thesis — 지난주 판세를 1~2문장으로 선언. 반드시 “지난주 초반 daily에서 보던 흐름과 주 후반에 무엇이 달라졌는지”를 포함한다.
...
> 푸터: 이 사이트는 라이브 발행본이다. `index.html`과 모든 포스트 푸터에 "미리보기/발행 전/preview" 같은 비공개 프레이밍을 넣지 않는다(과거 footer 문구 재생성 금지). 푸터가 필요하면 입력 출처·RSS 링크 정도만 둔다.
...
생성과 Slack은 **같은 실행에서** 처리한다(별도 발송 잡 없음). 정기 실행은 두 종류다:
- **daily 실행** — 매일 11:00 KST(`0 2 * * *`). arXiv에 새 listing이 있으면 daily를 생성하고, push 성공 직후 **daily 요약** Slack을 보낸다.
- **weekly 실행** — 매주 월요일 10:00 KST(`0 1 * * 1`, `mode=weekly`). 지난주 회고 weekly를 생성하고, push 성공 직후 **weekly 요약** Slack을 보낸다.

에이전트는 두 경우 모두 메시지 본문(mrkdwn)을 `out/slack_message.txt`로 작성한다(daily면 daily 요약, weekly면 weekly 요약). 같은 실행의 마지막 단계가 이를 **Slack 봇(`chat.postMessage`)**으로 `SLACK_CHANNEL_ID` 채널에 보낸다. **발행 시(`out/release_ok.txt` 존재)** 요약을, **실행 실패 시** 실패 알림을 보내며, **no-op(주말 daily 등)은 미발송**이다.
