...
push 성공 직후 발송한다. 실제 발송은 워크플로우가 **Slack 봇(`chat.postMessage`)**으로 `SLACK_CHANNEL_ID` 채널에 수행하며(**토·일 KST은 발송 안 함**), 에이전트는 메시지 본문(mrkdwn)을 `out/slack_message.txt`로 작성한다.
